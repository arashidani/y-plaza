    ...
    <main className="pool-calculator-container">
      ...
      <fieldset className="mt-6">
        <legend className="sr-only">
          {t('poolCalculator.selectServices', {
          ...
          {[
            {
              ...
              label: t('poolCalculator.type.pool', { defaultValue: 'プール' })
            },
            {
              type: 'gym' as LineType,
              label: t('poolCalculator.type.gym', {
                defaultValue: 'トレーニングジム'
              })
            },
            {
              type: 'locker' as LineType,
              label: t('poolCalculator.type.locker', {
                defaultValue: 'ロッカー'
              })
            },
            {
              type: 'membership' as LineType,
              label: t('poolCalculator.type.membership', {
                defaultValue: '会員券'
              })
            },
            {
              type: 'coupon' as LineType,
              label: t('poolCalculator.type.coupon', { defaultValue: '回数券' })
            }
          ].map(({ type, label }) => (
            <label
              key={type}
            ...
            >
              ...
              />
              <span id={`${type}-description`} className="text-sm">
                {label}
              </span>
            </label>
          ))}
          ...
          <div className="flex items-center gap-2 text-sm text-orange-800 dark:text-orange-200">
            ...
            <div className="flex-1">
              <div className="font-medium">
                {t('poolCalculator.lockerRequired', {
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-2" data-lazy-load>
        <Section
          title={t('poolCalculator.breakdown', { defaultValue: '内訳' })}
        >
          <div className="space-y-2">
            {[
              {
                key: 'pool',
                value: subtotal.pool,
                defaultLabel: 'プール入場料 小計'
              },
              {
                key: 'locker',
                value: subtotal.locker,
                defaultLabel: 'ロッカー 小計'
              },
              {
                key: 'gym',
                value: subtotal.gym,
                defaultLabel: 'トレーニングジム 小計'
              },
              {
                key: 'membership',
                value: subtotal.membership,
                defaultLabel: '会員券 小計'
              },
              {
                key: 'coupon',
                value: subtotal.coupon,
                defaultLabel: '回数券 小計'
              }
            ].map(
              (item) =>
                item.value > 0 && (
                  <SummaryRow
                    key={item.key}
                    label={t(`poolCalculator.subtotal.${item.key}`, {
            ...
            >
              <span className="font-semibold">
                {t('poolCalculator.total', { defaultValue: '合計' })}
              </span>
              <YenMono
                value={total}
        ...
        <Section
          title={t('poolCalculator.notes', { defaultValue: 'メモと注意事項' })}
        >
          <ul className="list-disc space-y-2 pl-5 text-sm text-gray-700">
            <li>
              <strong>
                {t('poolCalculator.note.lockerTitle', {
                  defaultValue: 'ロッカーについて：'
                })}
              </strong>{' '}
            ...
            <li>
              {t('poolCalculator.note.discount', {
                defaultValue:
                  '障害者割引は本人分の入場料金・会員券が半額（10円未満切り捨て）。備品関係は通常料金。'
              })}
            </li>
            <li>
              {t('poolCalculator.note.coupon', {
                defaultValue:
                  '回数券は 1冊=11枚綴りの購入金額のみを計上します（何回分消費するかの減算管理は含みません）。'
              })}
            </li>
            <li>
              {t('poolCalculator.note.family', {
                defaultValue: '会員の家族等の特殊区分は未対応です。'
              })}
            </li>
          </ul>
        </Section>
